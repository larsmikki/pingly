...
export function Modal({ open, onClose, title, children, maxWidth = '640px' }: Props) {
  ...
  return (
    ...
    >
      <div
        className="w-full mx-4 rounded-2xl shadow-modal overflow-hidden flex flex-col bg-surface border border-border"
        style={{ maxWidth, maxHeight: '90vh' }}
      >
        {title && (
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-border">
            <h3 className="text-base font-bold text-text">{title}</h3>
            <button onClick={onClose} aria-label="Close" className="p-1 text-text2 hover:opacity-70">
