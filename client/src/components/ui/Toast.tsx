...
function ToastItem({
  ...
  return (
    <div
      role={ariaRole}
      aria-live={ariaLive}
      className={`toast-slide-in toast-${toast.type} flex items-start gap-3 px-4 py-3 rounded-xl max-w-sm w-full pointer-events-auto text-text`}
    >
      ...
      <button
        ...
        className="shrink-0 p-0.5 text-text2 hover:opacity-70"
      >
...
function ToastIcon({ type }: { type: ToastType }) {
  if (type === 'success') {
    return (
      <svg className="w-5 h-5 shrink-0 mt-px text-success" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
  ...
  if (type === 'error') {
    return (
      <svg className="w-5 h-5 shrink-0 mt-px text-danger" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
  ...
  return (
    <svg className="w-5 h-5 shrink-0 mt-px text-accent" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
